...
[private vulnerability reporting](https://git.jaymr.top/Jaymr/genoffice/security/advisories/new)
...
- Every `shell.openExternal` call goes through a single shared gate
  (`@mroffice/electron-utils` → `safeExternalUrl`) that parses the URL and
