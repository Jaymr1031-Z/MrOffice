...
function blankFontFaceSrc(): string | null {
 ...
 ff.style.getPropertyValue('font-family').includes('MrOffice PUA Blank')
 ) {
 ...
 const m = /@font-face\s*{[^}]*MrOffice PUA Blank[^}]*?src:\s*([^;}]+)/.exec(
 el.textContent ?? '',
 )
...
export function docStyleCss(parsed: ParsedDocFull): string {
 ...
 gridStrut = true
 rules.push(
 `@font-face { font-family:'MrOffice Grid Strut'; src:${blankSrc}; ` +
 ...
 rules.push(
 `.doc-page .doc-grid-strut { font-family:'MrOffice Grid Strut',var(--doc-grid-strut-tail,serif) }`,
 )
 }
