...
import type { DocGrid } from '@mroffice/docx-engine'
...
export function cssFontFamily(font: string, followAltName = true): string {
 ...
 // 'MrOffice Box Drawing' (fonts.css): U+2500 rules would otherwise fall to
 // the fullwidth CJK subset and overflow the column
 const BOX = 'MrOffice Box Drawing'
 ...
 // 'MrOffice PUA Blank' keeps AI-residue PUA tokens invisible like Word
 ...
 ? 'Aptos Display GO'
 : 'Aptos GO'
 return `${chain(font, alias, 'MrOffice PUA Blank', CJK_SANS)},sans-serif`
 }
 ...
 if (f.includes('poppins')) return `${chain(font, 'MrOffice Poppins', CJK_SANS)},sans-serif`
 ...
 // 'MrOffice Songti SC' (fonts.css local() alias of Songti SC): macOS Chromium
 ...
 return `${chain(font, 'MrOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF)},serif`
 }
 ...
 // 'MrOffice *' entries are CJK-only local() aliases (fonts.css): the underlying
 // system faces draw Cyrillic/Greek fullwidth, so those scripts must pass through
 const JA_SANS = ['Yu Gothic', 'MrOffice Hiragino Sans', 'Meiryo', 'Noto Sans JP']
 const JA_SERIF = [
 'Yu Mincho',
 'MrOffice Hiragino Mincho',
 'MrOffice MS Mincho',
 'Noto Serif JP',
 ]
 const KO_SANS = ['Malgun Gothic', 'MrOffice Sans KR', 'Apple SD Gothic Neo', 'Noto Sans KR']
 const KO_SERIF = ['MrOffice Batang', 'MrOffice Serif KR', 'MrOffice Myungjo', 'Noto Serif KR']
 const TC_SANS = ['Microsoft JhengHei', 'PingFang TC', 'MrOffice Heiti TC', 'Noto Sans TC']
 // 'MrOffice Fullwidth TC' (fonts.css): fullwidth U+FF0D/FF0F/FF3C/FF3F/FF5E whose Songti TC glyphs look half-width
 const TC_SERIF = ['MrOffice MingLiU', 'MrOffice Fullwidth TC', 'Songti TC', 'Noto Serif TC']
 const SC_SANS = ['PingFang SC', 'Microsoft YaHei', CJK_SANS]
 const SC_SERIF = ['MrOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF]
 const nfkc = font.normalize('NFKC')
 ...
 const hangulTail = !isKr && missingLocally() ? ['MrOffice Batang', 'MrOffice Serif KR'] : []
 ...
 return `${chain(font, 'MrOffice Gothic KR', ...KO_SANS)},sans-serif`
 }
 ...
 return `${chain(font, 'MrOffice Che Latin KR', ...(cheGungsuh ? ['GungSeo'] : []), ...(cheSerif ? KO_SERIF : KO_SANS))},${cheSerif ? 'serif' : 'sans-serif'}`
 }
 ...
 return `${chain(font, 'MrOffice Ethiopic')},sans-serif`
 }
 ...
 return `${chain(font, 'MrOffice Tamil', 'InaiMathi', 'Tamil MN', 'Tamil Sangam MN')},sans-serif`
 }
 ...
 const pua = latinNamed ? ['MrOffice PUA Blank'] : []
