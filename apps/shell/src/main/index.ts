...
const tMain = createI18n({
  zh: {
    ...
    menuSectionNew: '新建',
    menuNewDoc: 'AI Docs',
    menuNewSheet: 'GenOffice Sheets',
    untitledSheet: '未命名表格',
    untitledDoc: '未命名文档',
    untitledDeck: '未命名演示文稿',
    menuNewSlide: 'GenOffice Slides',
    menuOpen: '打开…',
    menuSave: '保存',
    menuSaveAs: '另存为…',
    menuClose: '关闭',
...
function registerHomeIpc(): void {
  ...
  ipcMain.handle(
    ...
    // before the campaign launch time, report "seen" to suppress the
    // onboarding without persisting the flag — users who have not seen it
    // get it on their first launch after the gate opens
    (): boolean =>
      !isOnboardingLive(Date.now()) || readAppSettings(APP_SETTINGS_PATH()).onboardingSeen === true,
  )
...
app.on('before-quit', () => {
  // No close prompt may fall through to "Save" during shutdown (issue #170)
  markSheetsShuttingDown()
  stopSheetsSidecar()
})
