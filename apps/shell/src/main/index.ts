...
/**
 * MrOffice unified shell: ONE Electron app, ONE BrowserWindow, hosting the
...
// run silently quits and forwards its argv to the running installed MrOffice.
...
if (!app.isPackaged)
 app.setPath(
 'userData',
 process.env.GENOFFICE_USER_DATA ?? join(app.getPath('appData'), 'MrOffice Dev'),
 )

// The product rename from "AI Office" to MrOffice changed the userData path; migrate old user data once
if (app.isPackaged) {
...
// The "community" link previously opened a remote server page. MrOffice is
// local-first, so no outbound community link is wired up.
const GENTEAM_URL = ''
...
// Stargazer count for the settings About pane. MrOffice is local-first and
// must not phone home to any remote server, so the remote count is disabled
// and the About pane simply shows no star number.
...
function registerHomeIpc(): void {
 ...
 ipcMain.handle(HOME_CHANNELS.openMrOffice, () => {
   if (GENTEAM_URL) {
...
// is unaffected. The bootstrap is shared by the editor main processes.
...
async function installMainProcessProxy(): Promise<void> {
 ...
 if (!proxyUrl) {
 try {
 // PAC/rule proxies answer per-host: probe a representative HTTPS API host.
