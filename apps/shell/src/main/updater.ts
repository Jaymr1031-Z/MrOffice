...
import { createI18n, getUiLang, htmlLang } from '@mroffice/i18n'
...
// Last-resort manual download link. MrOffice is local-first and ships no
// remote update server, so this is intentionally empty; manualDownloadUrlFor
// (derived from a baked-in update feed, if any) is preferred when available.
...
export function initAutoUpdater(
 ...
 onOpenDownload: () => {
   const url = manualDownloadUrl ?? DOWNLOAD_PAGE_URL
