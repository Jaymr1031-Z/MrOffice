import type { AgentSkill, ToolDisplay } from '@mroffice/agent-core'
import type {
 GroupRenderNode,
 PictureRenderNode,
 RenderNode,
 ...
 */
 ...
 /** A single LLM request writes a structured slide spec that the main process builds into PPTX. */
...
const AGENT_SYSTEM_PROMPT = `You are the AI assistant inside MrOffice Slides (a slide editor), helping users improve and generate presentations.
...
- Editing an EXISTING picture: apply_ops setPictureSrcRect (non-destructive crop, fractions 0..1) and setPictureOpacity, or replace_image with a replacement URL from image_search. Never delete+reinsert a picture to change its content — that loses z-order and effects.
...
const TOOLS: AgentToolDef[] = [
 ...
 name: 'replace_image',
 description:
 'Swap a picture\'s source image for a URL from image_search in place — position, size, z-order, border and effects all survive. A new image with a different aspect ratio is never stretched: it fills the frame and is center-cropped (object-fit: cover). keepCrop keeps the existing crop window and is only correct when the new image has the same pixel geometry as the old one.',
 inputSchema: {
 type: 'object',
 properties: {
 slideIndex: { type: 'integer' },
...
async function executeTool(
 ...
 : []
 // One retry then give up (same semantics as generate_deck pages). The marker points at a
 // one-slide PPTX temp file; landing is shared.
 ...
 // The configured AI provider writes a slide spec that is built directly into a PPTX.
 ...
 // Generation returns a marker pointing at a one-slide PPTX temp file. One retry, then
 // the page is skipped for now and the rest of the deck keeps generating.
 ...
 // Landing-failed pages re-land; generation failures get one more local-spec attempt.
 if (!cancelled()) {
