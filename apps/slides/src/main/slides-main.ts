/**
 * MrOffice Slides main process — pptx parsing/render-tree building/edit application/saving all live
...
/**
 * Recovery drafts for never-saved decks (wcId → visible path in <Documents>/MrOffice):
...
/** Directory where AI-generated drafts are saved: the configurable default save folder (falls back to <Documents>/MrOffice) */
...
/**
 * Auto-save the draft to <Documents>/MrOffice/<name>.pptx after AI generation completes.
...
export function registerSlidesIpc(): void {
 ...
 // ── Local single-page generation: a JSON slide spec written by the renderer's LLM call is
 // built directly into a one-slide pptx with pptx-engine primitives — no HTML intermediate.
 ipcMain.handle(
