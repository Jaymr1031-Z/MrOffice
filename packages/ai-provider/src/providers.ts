 ...
 * settings file — falls back to Codex so a half-filled setup does not silently
 * select a network service or account.
 */
export function activeProvider(settings: AiSettings): AiProviderId {
  ...
  if (meta.needsBaseUrl) {
    ...
    if (!config.baseUrl) return 'codex'
    return provider
  }
  if (!config.apiKey) return 'codex'
  return provider
}
